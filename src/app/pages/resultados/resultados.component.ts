...
import { ResultadosService } from 'src/app/services/services.index';
...
export class ResultadosComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    init_plugins();
    this.cambiarTab(this.resultadosService.tabselected);
    this.scrollTop(); // envio el scroll hacia arriba
  }

  tabSelected(n: number) {
    this.resultadosService.tabselected = n;
  }
  ...
  onWindowScroll() {
    ...
    if (((document.documentElement.scrollTop + document.documentElement.clientHeight) * 100 / contentHeight) > this.INFINITESCROLL_THRESHOLD) {
      if (this.getMoreProps === false) {
        this.resultadosService.cargarPropiedades();
      }
